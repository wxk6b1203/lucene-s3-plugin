...
- JDK 25，启动时需要添加 `--add-modules jdk.incubator.vector` 参数以启用 Lucene 的向量化加速。
- Gradle Wrapper。
- 多节点模式需要 etcd。
...
发行包输出到 `server/build/distributions/server-1.0-SNAPSHOT.zip`。Windows 启动脚本 `bin/server.bat` 使用 `lib/server-1.0-SNAPSHOT-classpath.jar` 作为 pathing jar，避免把所有依赖 jar 展开到命令行导致 classpath 过长。启动脚本会自动包含 `--add-modules jdk.incubator.vector` 参数。
